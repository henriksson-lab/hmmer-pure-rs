...
## Post-merge parity pass after PR #2 - 2026-09-19 (release 0.8.0)

Follow-up to the single-sequence builder merge. Every item below was verified
byte-for-byte (or exit-code-and-message) against the bundled C binaries and is
pinned by a test in `tests/cli_output_parity.rs` unless noted. Deliberate
deviations from C now live in `BUGS.md`.

Fixed:

- **`-A` alignment output ignored `--notextw`.** C writes Stockholm in
  200-column blocks when `textw > 0` and one unwrapped Pfam block under
  `--notextw` (hmmsearch.c:566, phmmer.c:630, jackhmmer.c:723, nhmmer.c:1181).
  hmmsearch/phmmer always wrapped, jackhmmer never wrapped, and nhmmer had its
  own writer that never wrapped. All four now share one writer keyed on
  `textw` (`stockholm_cpl_for_textw`). Closes the `phmmer -A` item below.
- **nhmmer `-A` had two more divergences** surfaced by the wrap fix: hit rows
  were named `seq/min-max` where C uses alignment orientation
  (`seq/40318-40002` for a minus-strand hit, p7_alidisplay.c:1319), and the MSA
  kept every consensus column where C's nhmmer passes `p7_DEFAULT`
  (nhmmer.c:1174) and drops match columns no hit covers. `included_alignment`
  now takes an `all_consensus_cols` flag (hmmsearch/phmmer/jackhmmer keep
  passing `p7_ALL_CONSENSUS_COLS` semantics). Test
  `nhmmer_alignment_output_wraps_like_bundled_c` (synthetic 320 bp query in a
  100 kb target, one minus-strand hit, both textw modes).
- **`#=GS ... AC` lines were missing** from every `-A`/checkpoint Stockholm
  file when targets carry accessions (UniProt/EMBL input). Now emitted in
  Easel's order (AC block, then DE block).
- **Calibration failure was swallowed.** `calibrate_msv/viterbi/forward` had
  finiteness filters, `< 10` guards and `unwrap_or(-3.0)` fallbacks that C does
  not have, so a model whose Gumbel fit fails searched with fake STATS. They now
  return `Result` with C's errbuf text; `Builder::single_builder` and the
  `build_hmm_from_msa*` family return `Result<Hmm, String>` (API change, part
  of the 0.8.0 break), and hmmbuild/phmmer/jackhmmer/nhmmer exit 1 with
  `build failed: %s` like C. `p7_ForwardParser`'s NaN -> `eslERANGE` is
  mirrored too.
- **hmmbuild/nhmmer forced hand architecture for A2M input.** Easel's A2M
  reader supplies an implied RF line, but C only honours RF under `--hand`, so
  with the default `--symfrac` a lowercase A2M column can still become a match
  state (C: 7 nodes on the toy fixture; this port built 6). Two tests had
  encoded the wrong value and were corrected.
- **hmmalign `--outformat A2M` dropped the sequence accession** (`>name acc
  desc`, esl_msafile_a2m.c:418-420).
- **hmmbuild summary table printed the model name, not the MSA name**
  (hmmbuild.c:1294 prints `msa->name`); differs under `--singlemx`.
- **nhmmscan accepted a pressed DB without MAXL** and recomputed a window;
  C refuses it (nhmmscan.c:360).
- Fixture download scripts pass `--http1.1` to curl (UniProt HTTP/2 failure).

Still open (in priority order):

- ~~`ctime`/`DATE` is never set on any build path.~~ Decided 2026-09-19: keep
  reproducible output; recorded in `BUGS.md`.
- **Fixtures missing from a clean checkout** (`hmmer/testsuite/minipfam.hmm`,
  `gecco_*`; ~110 test references). **Partly resolved 2026-09-19:** all six
  HMM fixtures turned out to be `hmmfetch` pulls from pinned Pfam releases
  (35.0; minipfam from 34.0) and are regenerated byte-for-byte by
  `scripts/download_testsuite_fixtures.sh` (see `REAL_WORLD_FIXTURES.md`).
  The five `gecco_*proteins.faa` files (GECCO gene predictions on GenBank
  CP157504.1, 23 KB total) have no source and now live in `test_data/`.
  **Resolved 2026-09-20.**
- ~~Check ordering vs C for `--mx`/`--mxfile` errors vs missing input files.~~
  Out of scope for the translation (decided 2026-09-19); recorded in `BUGS.md`.
- `phmmer.c:557` and `jackhmmer.c:683` do not check the builder's status at
  all; this port reports the failure. Kept deliberately (see the calibration
  item above); not listed in `BUGS.md` because C's behaviour there is
  undefined rather than different.
- The `seqmodel::build_single_seq_hmm` compatibility wrapper still returns a
  bare `Hmm` and panics on calibration failure; library users should move to
  `Builder`.
- dsqdata `taxid` storage and makehmmerdb multi-block overlap ordering
  (unchanged, minor).
...
- **UniProt's stream endpoint fails over HTTP/2.** **FIXED 2026-09-19** (both scripts now pass `--http1.1`). Both proteome downloads in
...
- **nhmmscan with a `MAXL`-less model**: **FIXED 2026-09-19** (nhmmscan now refuses the database with C's message; test `nhmmscan_rejects_models_without_maxl_like_bundled_c`). C fails hard ("No MAXL field in
  model(s); is this an old model format?", nhmmscan.c:360) where this port
  recomputes from the default tail mass.
- **`phmmer -A` wraps Stockholm output; C does not (found in this pass).** **FIXED 2026-09-19** (hmmsearch/phmmer/jackhmmer/nhmmer `-A` now pick Stockholm-200 vs Pfam from `textw`, see the 2026-09-19 section).
...
- **Calibration failure is not propagated (HIGH, found in this pass).** **FIXED 2026-09-19** (`calibrate_with_config` and the MSA builders now return `Result`; drivers exit 1 with C's `build failed: %s`; test `nhmmer_reports_calibration_failure_like_bundled_c`). When a
...
- **jackhmmer**: two integration tests fail against the current **FIXED 2026-09-19** (both tests pass on the current tree; the suite is green).
...
- **phmmer/hmmsearch `-A`** dumped raw `aseq` instead of a real
  ...
  **`#=GS AC` FIXED 2026-09-19** (shared writer emits Easel's AC block; test
  `hmmsearch_alignment_output_carries_accessions_like_bundled_c`). The PP_cons
  item could not be reproduced on the current tree: `-A` output is byte-identical
  to C on the E. coli DnaK, fn3/7LESS_DROME and MADE1 fixtures.
