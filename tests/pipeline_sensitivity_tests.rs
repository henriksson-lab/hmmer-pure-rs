...
fn run_pipeline(hmm: &hmmer_pure_rs::hmm::Hmm, seq: &[u8]) -> Vec<(usize, usize, f32)> {
    ...
    let sq = sequence::Sequence {
        name: "target".to_string(), acc: String::new(), desc: String::new(),
        dsq, n: seq.len(), l: seq.len(),
    };
    pli.run(&mut gm, &mut om, &bg_obj, hmm, &sq, &mut th);
    th.hits.iter().flat_map(|h| h.dcl.iter().map(|d| (d.ienv as usize, d.jenv as usize, d.bitscore))).collect()
}

fn infernal_testsuite() -> std::path::PathBuf {
    for p in ["/data/henriksson/github/claude/infernal-rs/infernal/testsuite", "../infernal-rs/infernal/testsuite"] {
        let path = Path::new(p);
        if path.join("tRNA.c.cm").exists() { return path.to_path_buf(); }
    }
...
fn pipeline_finds_trna_76bp() {
    ...
    for (i, j, sc) in &domains { println!("  {}-{}: {:.1}", i, j, sc); }
    assert!(!domains.is_empty(), "Pipeline should find tRNA in 76bp");
    assert!(domains[0].2 > 20.0, "Score should be >20, got {:.1}", domains[0].2);
}
...
fn pipeline_finds_trna_1000bp() {
    ...
    for (i, j, sc) in &domains { println!("  {}-{}: {:.1}", i, j, sc); }
    assert!(domains.iter().any(|(i, _, sc)| *i >= 830 && *i <= 900 && *sc > 10.0),
...
fn ssv_sensitivity_ecoli_genome() {
    ...
    if !ecoli_path.exists() { println!("Skipping: /tmp/ecoli_k12.fna not found"); return; }
    ...
    let windows = unsafe {
        simd::ssv_longtarget::ssv_filter_longtarget(
            &dsq, genome.len(), &om, &bg_obj, 0.02, (hmm.m * 4) as i32,
        )
    };

    println!("SSV on E. coli ({}bp): {} windows", genome.len(), windows.len());
    ...
    assert!(windows.len() >= 50,
        "SSV should find at least 50 windows in E. coli, got {}", windows.len());
}
