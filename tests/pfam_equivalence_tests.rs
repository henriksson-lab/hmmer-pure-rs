...
//!
//! These tests use local/generated Pfam HMM fixtures and search them against
//! 20k human Swiss-Prot proteins. Golden reference files were generated by C
//! HMMER 3.4.
//!
//! Tests verify:
...
//! which are generated by running C hmmsearch on local/generated Pfam HMMs.
...
fn require_local_fixture(path: &str) {
    assert!(
        std::path::Path::new(path).exists(),
        "missing local/generated Pfam fixture: {}",
        path
    );
}
...
// Prefer uncompressed for speed; fall back to the local/generated .gz fixture.
fn seqdb_path() -> String {
    ...
    if std::path::Path::new(&gz).exists() {
        ...
        let tmp = std::env::temp_dir().join("hmmer-rs-human_swissprot_2k.fasta");
        std::fs::write(&tmp, &data).unwrap();
        return tmp.display().to_string();
    }
    panic!(
        "missing local/generated Pfam sequence fixture: {} or {}",
        uncompressed, gz
    );
}
...
macro_rules! pfam_test {
    ($test_name:ident, $family:expr, $min_overlap:expr, $max_strong_missed:expr, $max_score_diff:expr) => {
        ...
        fn $test_name() {
            ...
            require_local_fixture(&golden);
            require_local_fixture(&hmm);
            require_local_fixture(&seqdb);
