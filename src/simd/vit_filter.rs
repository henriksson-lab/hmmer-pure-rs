...
mod tests {
    ...
    fn test_viterbi_filter_matches_ffi() {
        ...
        let rust_result = unsafe { viterbi_filter(&dsq, l, &om) };
        ...
        unsafe {
            ...
            let c_status = crate::ffi::p7_ViterbiFilter(c_dsq, l as i32, c_om, c_ox, &mut c_sc);

            match rust_result {
                VitResult::Ok(rust_sc) => {
                    if c_status == 0 {
                        assert!(
                            (rust_sc - c_sc).abs() < 1.0,
                            "Viterbi scores differ: rust={}, c={}",
                            rust_sc,
                            c_sc
                        );
                    }
                ...
                VitResult::Overflow => {
                    assert!(c_status == 16, "Expected overflow in C too");
                }
            }
