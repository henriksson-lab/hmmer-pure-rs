...
mod tests {
    ...
    fn test_digitize_matches_ffi() {
        ...
        unsafe {
            let c_abc = crate::ffi::esl_alphabet_Create(AlphabetType::Amino as i32);
            assert!(!c_abc.is_null());

            // Compare K and Kp
            assert_eq!(abc.k, (*c_abc).K as usize);
            assert_eq!(abc.kp, (*c_abc).Kp as usize);

            // Compare inmap for all ASCII characters
            for ch in 0..128u8 {
                let rust_code = abc.inmap[ch as usize];
                let c_code = (*c_abc).inmap[ch as usize];
                assert_eq!(
                    rust_code, c_code,
                    "inmap mismatch for char '{}' ({}): rust={}, c={}",
                    ch as char, ch, rust_code, c_code
                );
            }
            ...
            for i in 0..abc.kp {
                assert_eq!(
                    abc.sym[i], *(*c_abc).sym.add(i) as u8,
                    "sym mismatch at {}: rust={}, c={}",
                    i, abc.sym[i] as char, *(*c_abc).sym.add(i) as u8 as char
                );
            }
    ...
    fn test_digitize_dna_matches_ffi() {
        let abc = Alphabet::dna();
        unsafe {
            let c_abc = crate::ffi::esl_alphabet_Create(AlphabetType::Dna as i32);
            assert!(!c_abc.is_null());
            ...
            for ch in 0..128u8 {
                ...
                assert_eq!(
                    rust_code, c_code,
                    "DNA inmap mismatch for char '{}' ({}): rust={}, c={}",
                    ch as char, ch, rust_code, c_code
                );
            }
