...
mod tests {
    ...
    fn test_bg_null_one_matches_ffi() {
        ...
        let rust_sc = bg.null_one(200);

        unsafe {
            let c_abc = crate::ffi::esl_alphabet_Create(3); // eslAMINO = 3
            let c_bg = crate::ffi::p7_bg_Create(c_abc);
            crate::ffi::p7_bg_SetLength(c_bg, 200);
            let mut c_sc: f32 = 0.0;
            crate::ffi::p7_bg_NullOne(c_bg, std::ptr::null(), 200, &mut c_sc);

            assert!(
                (rust_sc - c_sc).abs() < 1e-5,
                "NullOne mismatch: rust={}, c={}",
                rust_sc,
                c_sc
            );
    ...
    fn test_bg_frequencies_match_ffi() {
        unsafe {
            ...
            for i in 0..20 {
                let c_f = *(*c_bg).f.add(i);
                assert!(
                    (bg.f[i] - c_f).abs() < 1e-6,
                    "bg.f[{}] mismatch: rust={}, c={}",
                    i,
                    bg.f[i],
                    c_f
                );
