...
mod tests {
    ...
    fn test_profile_config_matches_ffi() {
        ...
        unsafe {
            ...
            // Create and configure C profile
            let c_gm = crate::ffi::p7_profile_Create((*c_hmm).M, c_abc2);
            crate::ffi::p7_ProfileConfig(c_hmm, c_bg, c_gm, 400, P7_LOCAL);

            // Compare transition scores
            for k in 0..gm.m {
                for s in 0..P7P_NTRANS {
                    let r = gm.tsc(k, s);
                    let c = *(*c_gm).tsc.add(k * P7P_NTRANS + s);
            ...
            // Compare match emission scores for canonical residues
            for x in 0..abc.k {
                for k in 1..=gm.m {
                    let r = gm.msc(k, x);
                    let c = *(*(*c_gm).rsc.add(x)).add(k * P7P_NR + P7P_MSC);
                    if r.is_finite() && c.is_finite() {
                        assert!(
                            (r - c).abs() < 0.01,
                            "msc({},{}) mismatch: rust={}, c={}",
            ...
            // Compare special state transitions
            for s in 0..P7P_NXSTATES {
                for t in 0..P7P_NXTRANS {
                    let r = gm.xsc[s][t];
                    let c = (*c_gm).xsc[s][t];
                    if r.is_finite() && c.is_finite() {
                        assert!(
