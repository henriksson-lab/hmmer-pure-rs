...
mod tests {
    ...
    fn test_hmm_matches_ffi() {
        ...
        let rust_hmms = read_hmm_file(Path::new(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/hmmer/testsuite/20aa.hmm"
        )))
        .unwrap();
        let rust_hmm = &rust_hmms[0];

        unsafe {
            // Open and read via C FFI
            ...
            let status = crate::ffi::p7_hmmfile_Open(
                path.as_ptr(),
                std::ptr::null_mut(),
                &mut hfp,
                errbuf.as_mut_ptr(),
            );
            assert_eq!(status, 0, "Failed to open HMM file via FFI");

            let mut abc_ptr: *mut crate::ffi::ESL_ALPHABET = std::ptr::null_mut();
            let mut hmm_ptr: *mut crate::ffi::P7_HMM = std::ptr::null_mut();
            let status = crate::ffi::p7_hmmfile_Read(hfp, &mut abc_ptr, &mut hmm_ptr);
            assert_eq!(status, 0, "Failed to read HMM via FFI");

            let c_hmm = &*hmm_ptr;

            // Compare model length
            assert_eq!(rust_hmm.m, c_hmm.M as usize);

            // Compare name
            let c_name = CStr::from_ptr(c_hmm.name).to_str().unwrap();
            assert_eq!(rust_hmm.name, c_name);

            // Compare match emissions (node 1, all K values)
            let k = rust_hmm.abc_k;
            for i in 0..k {
                let c_val = *(*c_hmm.mat.add(1)).add(i);
                let r_val = rust_hmm.mat[1][i];
                assert!(
                    (r_val - c_val).abs() < 1e-4,
                    "mat[1][{}] mismatch: rust={}, c={}",
                    i,
                    r_val,
                    c_val
                );
            ...
            // Compare insert emissions (node 0)
            for i in 0..k {
                let c_val = *(*c_hmm.ins.add(0)).add(i);
                let r_val = rust_hmm.ins[0][i];
                assert!(
                    (r_val - c_val).abs() < 1e-4,
                    "ins[0][{}] mismatch: rust={}, c={}",
                    i,
                    r_val,
                    c_val
                );
            ...
            // Compare transitions (node 1)
            for i in 0..NTRANSITIONS {
                let c_val = *(*c_hmm.t.add(1)).add(i);
                let r_val = rust_hmm.t[1][i];
                assert!(
                    (r_val - c_val).abs() < 1e-4,
                    "t[1][{}] mismatch: rust={}, c={}",
                    i,
            ...
            // Compare E-value params
            for i in 0..NEVPARAM {
                assert!(
                    (rust_hmm.evparam[i] - c_hmm.evparam[i]).abs() < 1e-3,
                    "evparam[{}] mismatch: rust={}, c={}",
                    i,
                    rust_hmm.evparam[i],
                    c_hmm.evparam[i]
                );
            ...
            crate::ffi::p7_hmm_Destroy(hmm_ptr);
            crate::ffi::esl_alphabet_Destroy(abc_ptr);
